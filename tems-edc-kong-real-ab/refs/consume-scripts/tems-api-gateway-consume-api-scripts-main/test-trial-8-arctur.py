#!/usr/bin/env python3
"""
TEMS API Gateway Test Script - Trial 8 ARCTUR
==============================================

This script demonstrates the complete flow for using the TEMS API Gateway with the ARCTUR 3D Heritage API.

Real API endpoint to be tested trough API GATEWAY: https://repository.3d-heritage.eu/api/core/v1/assets/search
Query parameters: query=test&category=1&category=2&historicalPeriod=3&historicalPeriod=4&region=5&region=6&page.page=1&page.pageSize=20&portal=DIKD
...
# Initial banner
print_header("TEMS API Gateway Test - Trial 8 ARCTUR")
print("This script will test the TEMS API Gateway with the ARCTUR 3D Heritage API.\n")
...
# ============================================================================
# STEP 4: Test API call through API Gateway (ARCTUR 3D Heritage search endpoint)
# ============================================================================
print_step(4, "API Test Call through API Gateway - ARCTUR Assets Search")
print()

# Note: Kong config has strip_path: true, so we need to include the full path
original_endpoint = "https://repository.3d-heritage.eu/api/core/v1/assets/search?query=test&category=1&category=2&historicalPeriod=3&historicalPeriod=4&region=5&region=6&page.page=1&page.pageSize=20&portal=DIKD"
gateway_api_endpoint = f"{api_gateway_endpoint}/apigateway/api/core/v1/assets/search?query=test&category=1&category=2&historicalPeriod=3&historicalPeriod=4&region=5&region=6&page.page=1&page.pageSize=20&portal=DIKD"
...
print_info("The API Gateway will forward the request to the ARCTUR 3D Heritage API")
print_info(f"Using API Gateway token for authentication...")
print()
print_info("Search parameters:")
print_info("- query: test")
print_info("- categories: 1, 2")
print_info("- historicalPeriods: 3, 4")
print_info("- regions: 5, 6")
print_info("- page: 1, pageSize: 20")
print_info("- portal: DIKD")
print()
...
try:
    print_info("Sending GET request to /api/core/v1/assets/search...")
    ...
    if api_response.status_code == 200:
        ...
        try:
            ...
            # Show some stats if it's a search result
            if isinstance(response_json, dict):
                if "totalElements" in response_json:
                    print()
                    print_info(f"Total elements found: {response_json.get('totalElements', 'N/A')}")
                if "totalPages" in response_json:
                    print_info(f"Total pages: {response_json.get('totalPages', 'N/A')}")
                if "content" in response_json and isinstance(response_json["content"], list):
                    print_info(f"Results in this page: {len(response_json['content'])}")
...
print()
print(f"{Colors.CYAN}3. ARCTUR API CALL THROUGH GATEWAY{Colors.ENDC}")
print(f"   ✓ Successfully called ARCTUR 3D Heritage API through TEMS API Gateway")
print(f"   ✓ Tested endpoint: /api/core/v1/assets/search")
print(f"   ✓ Searched for 3D heritage assets with multiple filters")
print()
print(f"{Colors.GREEN}{Colors.BOLD}Success!{Colors.ENDC} {Colors.GREEN}TEMS API Gateway is working correctly with Trial 8 ARCTUR!{Colors.ENDC}")
print()
